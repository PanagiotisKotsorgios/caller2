...
$lead = [
    'id'=>0,'company_name'=>'','contact_name'=>'','phone'=>'','email'=>'','city'=>'','service_type'=>'website','status'=>'new','demo_sent'=>0,
    'follow_up_date'=>'','estimated_value'=>'','sale_value'=>'','sale_date'=>'','commission_percent'=>'','notes'=>'','assigned_to'=>$user['role']==='caller'?$user['id']:''
];
if($id){
...
$callers = $user['role']==='admin' ? $pdo->query("SELECT id,name,commission_percent FROM users WHERE role='caller' AND active=1 ORDER BY name")->fetchAll() : [];
$pageTitle = $id ? 'Edit Lead' : 'Add Lead';
include __DIR__ . '/includes/header.php';
?>
<div class="page-head"><div><h1><?= $id?'Edit lead':'Add lead' ?></h1><p><?= $id?'Update contact, pipeline and sale details.':'Create a new sales lead.' ?></p></div><a class="btn" href="leads.php">← Back</a></div>
<form method="post" action="lead_save.php" class="panel form-grid">
    ...
    <label>Assigned caller<select name="assigned_to"><option value="">Unassigned</option><?php foreach($callers as $c):?><option value="<?= (int)$c['id'] ?>" <?= (int)$lead['assigned_to']===(int)$c['id']?'selected':'' ?>><?= e($c['name']) ?> (<?= e($c['commission_percent']) ?>%)</option><?php endforeach;?></select></label>
    <label>Commission % for this sale<input type="number" name="commission_percent" step="0.01" min="0" max="100" value="<?= e((string)$lead['commission_percent']) ?>" placeholder="Blank = caller default until sold"></label>
    <?php else: ?>
    <div class="info-box"><strong>Your default commission:</strong> <?= e((string)$user['commission_percent']) ?>%. When a lead becomes Won, the rate is saved on that lead.</div>
    <?php endif; ?>
