...
$q=trim($_GET['q']??'');$status=trim($_GET['status']??'');$service=trim($_GET['service']??'');$callerRaw=$_GET['caller']??'';$where=[];$params=[];
...
fputcsv($out,['ID','Company','Contact','Phone','Email','Address','City','Region','Category','Subcategory','Source/comments','Service','Status','Demo sent','Follow up','Estimated EUR','Sale EUR','Sale date','Caller','Commission %','Notes']);
...
foreach($s as $r){$rate=$r['commission_percent']!==null?$r['commission_percent']:$r['caller_default'];fputcsv($out,[$r['id'],$r['company_name'],$r['contact_name'],$r['phone'],$r['email'],$r['address'],$r['city'],$r['region'],$r['category'],$r['subcategory'],$r['source_comments'],service_label($r['service_type']),status_label($r['status']),$r['demo_sent']?'Yes':'No',$r['follow_up_date'],$r['estimated_value'],$r['sale_value'],$r['sale_date'],$r['caller_name'],$rate,$r['notes']]);}
fclose($out);
