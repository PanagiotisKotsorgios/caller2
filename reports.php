...
$stmt=$pdo->prepare("SELECT u.id,u.name,u.commission_percent,
...
?>
<div class="page-head"><div><h1>Monthly commissions</h1><p>What you owe each caller for completed sales.</p></div><a class="btn primary" href="export.php?type=payouts&month=<?=e($month)?>">Export monthly payout CSV</a></div>
...
<section class="panel"><div class="panel-head"><h2>Caller payout summary · <?=e(date('F Y',strtotime($start)))?></h2></div><div class="table-wrap"><table><thead><tr><th>Caller</th><th>Sold clients</th><th>Sales value</th><th>Default rate</th><th>Amount owed</th></tr></thead><tbody><?php foreach($rows as $r):?><tr><td><strong><?=e($r['name'])?></strong></td><td><?=(int)$r['sold_count']?></td><td><?=money($r['revenue'])?></td><td><?=e($r['commission_percent'])?>%</td><td><strong><?=money($r['owed'])?></strong></td></tr><?php endforeach;?></tbody><tfoot><tr><th>Total</th><th></th><th><?=money($totalRevenue)?></th><th></th><th><?=money($totalOwed)?></th></tr></tfoot></table></div></section>
