...
$stmt = $pdo->prepare("SELECT l.*,u.name caller_name,u.commission_percent caller_commission,u.standard_price caller_standard_price FROM leads l LEFT JOIN users u ON u.id=l.assigned_to {$whereSql} ORDER BY l.updated_at DESC LIMIT {$perPage} OFFSET {$offset}");
$stmt->execute($params);
$leads = $stmt->fetchAll();

$callers = [];
if ($user['role'] === 'admin') $callers = $pdo->query("SELECT id,name,commission_percent,standard_price FROM users WHERE role='caller' AND active=1 ORDER BY name")->fetchAll();
...
<div class="page-head">
    ...
    <div class="actions"><a class="btn" href="export.php?type=leads&<?= e(build_query(['page'=>null])) ?>">Export CSV</a><?php if($user['role']==='admin'):?><a class="btn" href="import.php">Import XLSX</a><a class="btn primary" href="lead_form.php">+ Add lead</a><?php endif;?></div>
</div>
...
<form class="filters" method="get">
    ...
    <?php if ($user['role']==='admin'): ?><select name="caller"><option value="">All callers</option><option value="unassigned" <?= $caller==='unassigned'?'selected':'' ?>>Unassigned</option><?php foreach($callers as $c): ?><option value="<?= (int)$c['id'] ?>" <?= $caller===(int)$c['id']?'selected':'' ?>><?= e($c['name']) ?></option><?php endforeach; ?></select><?php endif; ?>
